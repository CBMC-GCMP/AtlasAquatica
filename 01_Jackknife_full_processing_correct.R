...
## in rds format
saveRDS(final_jackknifed_table, "results_iteration/final_jackknifed_table.RDS")
## in csv format
write_csv(final_jackknifed_table, "results_iteration/final_jackknifed_table.csv")
...
# creating message to slack
message <- glue_col("
                      The total revenue after {iterations_n} iterations is: 
                      
                      **{final_result_median} millions USD (q1 = {final_result_q1} millions, q3 = {final_result_q3} millions)**.
                      
                      The graph above visualize all the results from each iterations")


slackr::slackr_setup()

  
slackr::slackr_upload(filename = "results_iteration/iteration_plot_global.png", 
                        title = "Results of the iterations", 
                      initial_comment = message, 
                      channels = "data-stuff")



message2 <- glue_col("This is the final table")
...
slackr::slackr_upload(filename = "results_iteration/iteration_plot_regional.png", 
                      title = "Results of the iterations", 
                      initial_comment = message3, 
                      channels = "data-stuff")



message2 <- glue_col("This is the output table")


slackr::slackr_setup()


slackr::slackr_upload(filename = "results_iteration/final_jackknifed_table.csv", 
                      title = "Table output from the jackknife", 
                      initial_comment = message2, 
                      channels = "data-stuff")
